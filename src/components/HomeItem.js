    ...
    return (
        ...
        <View style = {{paddingHorizontal: 15, width: '100%', paddingTop: 40, paddingBottom: 10}}>
            <View style = {{width: '40%', alignItems: 'center', justifyContent: 'center', backgroundColor: colors.primary, borderRadius: 15, marginBottom: 10}}>
                <Text style = {{fontSize: 12, paddingVertical: 3, fontFamily: Fonts.OSR, color: 'white'}}>{tag}</Text>
            </View>
            <View style = {{marginBottom: 10}}>
                <Text style = {{fontSize: 16, fontFamily: Fonts.OSR}} numberOfLines = {3}>{title}</Text>
            </View>
            <View style = {{marginBottom: 20}}>
                <Text style = {{fontSize: 11, fontFamily: Fonts.OSR, color: 'rgba(180, 180, 180, 0.8)'}} numberOfLines = {2}>{content}</Text>
            </View>
            <View style = {{marginBottom: 10}}>
                <Text style = {{fontSize: 10, fontFamily: Fonts.OSR, color: colors.primary}}>@{location}</Text>
            </View>
        ...
        <View style = {{marginTop: 10, flexDirection: 'row', marginHorizontal: 10}}>
            <View style = {{flexDirection: 'row', width: '35%', justifyContent: 'space-between'}}>
                <View style = {{flexDirection: 'row', alignItems: 'center'}}>
                    ...
                    <Text style = {{color: 'rgba(150, 150, 150, 0.8)', fontFamily: Fonts.OSR, fontSize: 12}}> {seen}</Text>
                </View>
                <View style = {{flexDirection: 'row', alignItems: 'center'}}>
                    <EvilIcons name = 'like' style = {{fontSize: 20, color: isLiked ? colors.primary : 'rgba(150, 150, 150, 0.8)'}} onPress = {() => setIsLiked(!isLiked)} />
                    <Text style = {{color: isLiked ? colors.primary : 'rgba(150, 150, 150, 0.8)', fontFamily: Fonts.OSR, fontSize: 12}}> {seen}</Text>
                </View>
                <View style = {{flexDirection: 'row', alignItems: 'center'}}>
